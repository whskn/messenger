...
void render_side_bar(ui_t* ui_data) {
    ...
    for (int i = top_idx; 
         ...
         i++) 
    {
        if (i == ui_data->curr_chat) attron(COLOR_PAIR(CLR_CHAT_SELECTOR));
...
void render_msg_hist(ui_t* ui_data) {
    ...
    do { // iterate through history
        ...
        if (my_message) attron(COLOR_PAIR(CLR_MY_MSG_HEADER));
        else attron(COLOR_PAIR(CLR_MSG_HEADER));
        mvprintw(row + 1, LEFT_MSG_MARGIN, "%s", header);
        if (my_message) attroff(COLOR_PAIR(CLR_MY_MSG_HEADER));
        else attroff(COLOR_PAIR(CLR_MSG_HEADER));
