...
void ui_add_chat(ui_t* ui_data, chat_t* chat) {
    ...
    for (; idx < ui_data->n_of_chats; idx++) {
        if (chat->chat_id == ui_data->chats[idx].chat_id) break;
    }
...
void ui_remove_chat(ui_t* ui_data, chat_t* chat) {
    ...
    if (idx == ui_data->n_of_chats) return;
    ...
    ui_data->chats = (chat_t*)realloc(ui_data->chats, --ui_data->n_of_chats 
                                                    * sizeof(chat_t));
...
void ui_clear_history(ui_t* ui_data) {
    if (ui_data->history == NULL) return;

    for (int i = 0; i < MAX_HIST; i++) {
        if (ui_data->history[i]) free(ui_data->history[i]);
...
void ui_append_message(ui_t* ui_data, msg_t* msg) {
    ...
    if (ui_data->history[newhead]) free(ui_data->history[newhead]);
